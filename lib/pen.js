...
export default class Painter {
  ...
  paint(callback) {
    ...
    this.ctxS.draw(false, () => {
      setTimeout(() => {
        this.ctxD.draw(false, () => {
          callback();
        });
      }, 0);
    });
  ...
  translateTo(view, x, y) {
    this.ctx = this.ctxD;
    this.dynamicPool.forEach((v) => {
      if (v === view) {
        this._drawAbsolute(view, x, y);
      } else {
        this._drawAbsolute(view);
      }
    });
